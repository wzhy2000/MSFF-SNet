...
if __name__ == '__main__':
    # 模型路径
    model = 'output/models/240224_1303_train2/epoch_0499_iou_0.9906_class_0.7500_sort_0.7500'
    ...
    with torch.no_grad():
        for file in os.listdir(input_path):
            ...
            grasps, x1, y1 = affga.predict(img, device, mode='peak', thresh=0.1, peak_dist=1)	# 预测
            im_rest = drawGrasps(img, grasps, mode='arrow')  # 绘制预测结果
            rect = [x1, y1, x1 + 320, y1 + 320]

            # im_rest, x1, y1 = affga.maps(img,device)
            rect = [x1, y1, x1 + 320, y1 + 320]
            drawRect(im_rest, rect)
            # im_rest, x1, y1 = affga.maps(img, device)  # 预测
