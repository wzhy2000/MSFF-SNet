...
def parse_args():
    ...
    # 数据集
    parser.add_argument('--dataset-path', default='/home/plusmile/Desktop/affga-nets/dataset/cornell', type=str,
                        help='Path to dataset')
    parser.add_argument('--test-mode', type=str, default='image-wise', choices=['image-wise', 'object-wise', 'all-wise'],
                        help='测试方式')
    parser.add_argument('--data-list', type=str, default='train-test-cornell',
    ...
    parser.add_argument('--model', type=str, default='output/models/240406_0911_train2/epoch_0499_iou_0.9744_class_0.5128_sort_0.5030_', help='保存的模型')
    parser.add_argument('--start-epoch', type=int, default=479, help='继续训练开始的epoch')
...
def validate(net, device, val_data, saver, args):
    ...
    with torch.no_grad():  # 不计算梯度，不反向传播
        ...
        for x, y in val_data:
            ...
            able_out, angle_out, width_out, class_out = post_process_output(lossd['pred']['able'],
                                                                            lossd['pred']['angle'],
                                                                            lossd['pred']['width'],
                                                                            lossd['pred']['class'])
            # 保存预测图
            ...
            iou_flags, class_flags, succ_flags = evaluation(able_out, angle_out, width_out, class_out, y,
...
def run():
    ...
    for epoch in range(args.epochs)[start_epoch:]:
        ...
        if epoch % 10 == 9:
            ...
            if sort_accuracy >= best_acc:
                print('>>> save model: ',
                      'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f' % (epoch, accuracy, class_accuracy, sort_accuracy))
                saver.save_model(net, 'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f' % (
                epoch, accuracy, class_accuracy, sort_accuracy))

                best_acc = sort_accuracy
            else:
                print('>>> save model: ',
                      'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f_' % (epoch, accuracy, class_accuracy, sort_accuracy))
                saver.save_model(net, 'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f_' % (
                epoch, accuracy, class_accuracy, sort_accuracy))
