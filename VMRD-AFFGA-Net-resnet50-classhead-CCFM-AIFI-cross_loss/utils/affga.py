...
class AFFGA:
    ...
    def predict(self, img, device, mode, thresh=0.5, peak_dist=30, angle_k=120):
        """
        预测抓取模型
        ...
        # 后处理
        able_pred, angle_pred, width_pred,class_pred,class_confidence = post_process_output(self.able_out, self.angle_out, self.width_out, self.class_out)
        if mode == 'peak':
            # 置信度峰值 抓取点
            pred_pts = peak_local_max(able_pred,num_peaks_per_label=3, min_distance=peak_dist, threshold_abs=thresh,labels=class_pred)
        elif mode == 'all':
            # 超过阈值的所有抓取点
    ...
    def maps(self, img, device):
        ...
        self.net.eval()
        with torch.no_grad():
            self.able_out, self.angle_out, self.width_out,_ = get_pred(self.net, rgb.to(device))
        ...
        # able_featureMap[:, :, 1] = np.cos(able_map) * 255.0
