...
class GraspMat:
    ...
    def _decode(self, mat, angle_cls):
        ...
        # class_point = np.where(grasp_class > 0)
        # class_mat = np.zeros((1, h, w), dtype=np.float)
        # for i, _ in enumerate(class_point[0]):
        #     row, col = class_point[0][i], class_point[1][i]
        #     obj_class = grasp_class[row, col]
        #     class_mat[int(obj_class-1), row, col] = 1.

        grasp_confidence = np.expand_dims(grasp_confidence, axis=0)
        grasp_width = np.expand_dims(grasp_width, axis=0)
        ret_mat = np.zeros(shape=(123, 608, 608))   # 122,0,1~120,121,122~125
        ret_mat[0, :, :] = grasp_confidence
        ret_mat[1:121, :, :] = angle_mat
        ret_mat[121, :, :] = grasp_width / 600.
        ret_mat[122, :, :] = grasp_class
