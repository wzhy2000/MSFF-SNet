...
def evaluation(able_out, angle_out, width_out, class_out, class_confidence,target, angle_k, eval_mode, angle_th=30, iou_th=0.25,
               bottom=30, desc='1',):
    """
    评估预测结果
    ...
    angles_target = target[0, 1:121, :, :].cpu().numpy()  # (angle_k, 320, 320)
    width_target = target[0, 121, :, :].cpu().numpy() * 600.  # (320, 320)
    class_target = target[0, 122, :, :].cpu().numpy()

    # 搜索超过抓取置信度的待评估点
    threshold_abs = 0.3  # 0.3
    eval_mode = 'max'
    if eval_mode == 'peak':
        min_distance = 40
        ...
        num_peaks = len(np.unique(class_target))-1
        pred_pts = peak_local_max(able_out,  num_peaks=num_peaks,num_peaks_per_label=1,threshold_abs=threshold_abs,labels=class_out)
    ...
    for idx in range(pred_pts.shape[0]):
        ...
        cls_pred = class_out[row_pred, col_pred]  # 预测类别
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                # 3、15像素范围内，有一样的类
                if class_target[row, col] == cls_pred:
                    class_flag = 1



        iou_flags.append(iou_flag)
        class_flags.append(class_flag)
        if iou_flag:
            map_list.append([cls_pred,int(class_target[row_pred, col_pred]),class_confidence[row_pred, col_pred]])
        if iou_flag and class_flag:
            succ_flags.append(1)
        else:
            succ_flags.append(0)
    ...
    return iou_flags, class_flags, succ_flags,map_list
