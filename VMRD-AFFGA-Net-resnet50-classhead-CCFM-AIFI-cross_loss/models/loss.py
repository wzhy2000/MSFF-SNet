...
def compute_loss(net, xc, target, device):
    ...
    angle_target = target[:, 1:121, :, :].to(device)    # (-1, angle_k, 320, 320)
    width_target = target[:, 121, :, :].to(device)      # (-1, 320, 320)

    class_target = target[:, 122, :, :].to(device)

    able_pred, angle_pred, width_pred, class_pred = net(xc)         # shape 同上
    ...
    # class_loss = F.binary_cross_entropy(class_pred.squeeze(), class_target.squeeze())
    class_loss = F.cross_entropy(class_pred, class_target.long())
    ...
    return {
        'loss': able_loss + angle_loss * 5 + width_loss + class_loss * 10,
        'losses': {
            'able_loss': able_loss,
            'angle_loss': angle_loss * 5,
        ...
        'pred': {
            ...
            'class': class_pred,  # [-1, angle_k, 320, 320]
        }
    }


def get_pred(net, xc):
    able_pred, angle_pred, width_pred, class_pred = net(xc)
    """
