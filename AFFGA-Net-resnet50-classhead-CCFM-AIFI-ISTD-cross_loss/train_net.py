...
def parse_args():
    ...
    # 数据集
    parser.add_argument('--dataset-path', default='/home/plusmile/Desktop/affga-nets/dataset/istd', type=str,
                        help='Path to dataset')
    parser.add_argument('--test-mode', type=str, default='all-wise', choices=['image-wise', 'object-wise', 'all-wise'],
                        help='测试方式')
    parser.add_argument('--data-list', type=str, default='train-test-istd',
    ...
    parser.add_argument('--model', type=str, default='output/models/240227_2130_train2/epoch_0199_iou_0.9809_class_0.9708_sort_0.9708', help='保存的模型')
    parser.add_argument('--start-epoch', type=int, default=199, help='继续训练开始的epoch')
    ...
    for epoch in range(args.epochs)[start_epoch:]:
        ...
        if epoch % 10 == 9:
            ...
            if mAPg >= best_acc:
                ...
                saver.save_model(net, 'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f_mAPg_%0.4f' % (
                    epoch, accuracy, class_accuracy, sort_accuracy, mAPg))
                ...
                print('>>> save model: ',
                      'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f_mAPg_%0.4f_' % (
                          epoch, accuracy, class_accuracy, sort_accuracy, mAPg))
                saver.save_model(net, 'epoch_%04d_iou_%0.4f_class_%0.4f_sort_%0.4f_mAPg_%0.4f_' % (
                    epoch, accuracy, class_accuracy, sort_accuracy, mAPg))
