...
from models.affga.decoder_detr import build_decoder
from models.affga.hybrid_encoder import build_encoder
...
class DeepLab(nn.Module):
    ...
    def forward(self, input):
        ...
        # # able_pred, angle_pred, width_pred, class_pred = self.decoder(feat_1, hasp_small, hasp_big, x_all)

        # able_pred, angle_pred, width_pred, class_pred = self.decoder(outs)

        x, feat_1, feat_2, feat_3 = self.backbone(input)
        outs = self.encoder([feat_2, feat_3, x])
        able_pred, angle_pred, width_pred, class_pred = self.decoder(outs)
        ...
        class_pred = F.interpolate(class_pred, size=input.size()[2:], mode='bilinear', align_corners=True)
