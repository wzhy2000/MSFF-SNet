...
def evaluation(able_out, angle_out, width_out, class_out, class_confidence,target, angle_k, eval_mode, angle_th=30, iou_th=0.25,
    ...
    width_target = target[0, 121, :, :].cpu().numpy() * 200.  # (320, 320)
    ...
    if eval_mode == 'peak':
        ...
        num_peaks = len(np.unique(class_out))-1
        pred_pts = peak_local_max(able_out,  num_peaks=num_peaks,threshold_abs=threshold_abs,labels=class_out)
