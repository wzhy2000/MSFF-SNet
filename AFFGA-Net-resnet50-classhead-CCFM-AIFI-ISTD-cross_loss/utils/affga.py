...
class AFFGA:
    ...
    def predict(self, img, device, mode, thresh=0.5, peak_dist=3, angle_k=120):
        ...
        # 后处理
        able_pred, angle_pred, width_pred,class_pred,_ = post_process_output(self.able_out, self.angle_out, self.width_out, self.class_out)
        if mode == 'peak':
            # 置信度峰值 抓取点
            pred_pts = peak_local_max(able_pred,num_peaks_per_label=5, min_distance=peak_dist, threshold_abs=thresh,labels=class_pred)
        elif mode == 'all':
            # 超过阈值的所有抓取点
